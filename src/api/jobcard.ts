...
app.post("/create", async (req: Request, res: Response) => {
  try {
    ...
    io.emit(
      `new-job-card-${checkIfAppointmentExist.serviceCenterId}`,
      await encryptSocketData(JSON.stringify(newJobCard))
    );
...
app.delete("/delete/:jobCardId", async (req: Request, res: Response) => {
  try {
...
app.post(`/part/add`, async function (request: Request, response: Response) {
  try {
    ...
    if (quantity < 0) {
      return response.status(400).send("Quantity should be greater than zero");
    }
    ...
    let newJobCardPart = await prisma.$transaction(async (tx) => {
      let newPart = await tx.jobCardParts.create({
      ...
      await tx.jobCards.update({
        ...
        data: {
          price: {
            increment: newPart.partUsed.unitPrice * qty,
          },
        },
      });
    ...
    io.emit(
      `new-job-card-part-${existingJobCard.appointment.serviceCenterId}`,
      await encryptSocketData(JSON.stringify(newJobCardPart))
    );

    return response.status(200).send("Part Added Successfully");
  } catch (error) {
